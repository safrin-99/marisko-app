...
export default function CutiPage() {
  ...
  // =========================================================================
  // SAKTI: MESIN IN-APP NOTIFICATION (DENGAN DURASI LEBIH LAMA)
  // =========================================================================
  useEffect(() => {
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'cuti_history' }, (payload) => {
        ...
        if (newData && newData.status) {
          if (userRole === 'KARYAWAN') {
            ...
            // SAKTI: Durasi diperlama menjadi 15 detik sesuai perintah Komandan
            setTimeout(() => {
              setLiveNotif(null);
            }, 15000); 
          }
                  ...
                  historyData.filter((row) => {
                    ...
                    return (
                      <tr key={index} className="hover:bg-indigo-50/40 transition-colors">
                        ...
                        <td className="px-6 py-5 whitespace-nowrap"><div className="font-bold text-slate-700">{formatDateTime(null, row.tglMulai).date} - {formatDateTime(null, row.tglSelesai).date}</div></td>
                        <td className="px-6 py-5 whitespace-nowrap">
                          <div className="flex items-center justify-end gap-2.5">
                            ...
                            {/* SAKTI: HANYA MUNCULKAN EDIT JIKA BUKAN KARYAWAN ATAU STATUS MASIH PROSES */}
                            {(userRole !== 'KARYAWAN' || statusCuti === 'DIPROSES') && (
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden transition-all duration-300">
            ...
            <div className="overflow-y-auto overflow-x-auto max-h-[420px] scrollbar-thin">
              <table className="w-full text-sm text-left border-collapse whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-100">
                  {rekapCutiData.length === 0 ? (<tr><td colSpan="6" className="text-center py-8 text-slate-400 font-medium">Belum ada rekap data cuti/izin yang disetujui.</td></tr>) : (
