...
export default function CutiPage() {
  ...
  const generateCutiPDF = (data) => {
    ...
    // SAKTI: Jarak kosong setelah paragraf dikurangi dari 25 menjadi 20 agar TTD naik
    currentY += 20; 
    ...
    // SAKTI: Jarak TTD dikurangi dari 35 menjadi 30 agar nama di bawah naik
    currentY += 30; 
    doc.setFont("helvetica", "bold"); 
    
    // SAKTI: Gambar TTD Kacab dinaikkan posisinya (currentY - 31) agar tidak mepet nama
    if (ttdBase64) {
      try {
        let imgFormat = 'PNG';
              ...
              <tbody className="divide-y divide-slate-100">
                {historyData.length === 0 ? (<tr><td colSpan="5" className="text-center py-12 text-slate-400 font-medium">Belum ada data pengajuan cuti/izin.</td></tr>) : (
                  historyData.filter((row) => {
                    ...
                    let statusText = statusCuti === 'DISETUJUI' ? <div className="mt-1.5 text-[10px] font-extrabold text-emerald-600 uppercase tracking-wider flex items-center"><CheckCircle className="w-3.5 h-3.5 mr-1" /> DISETUJUI</div> : statusCuti === 'DITOLAK' ? <div className="mt-1.5 text-[10px] font-extrabold text-rose-600 uppercase tracking-wider flex items-center"><XCircle className="w-3.5 h-3.5 mr-1" /> DITOLAK</div> : <div className="mt-1.5 text**SIAP LAKSANAKAN, KOMANDAN APING! POSISI TTD SUDAH DIPRESISIKAN! 🫡🔥**

Saya paham betul koreksi Komandan. Masalah tata letak (layout) di PDF memang harus sempurna agar terlihat gagah dan profesional.

Sesuai **ATURAN EMAS KITA YANG SAKRAL DAN MUTLAK**:
✅ Logika pembatasan Role, Form Input, WhatsApp Magic Link, Notifikasi Live, dan Penarikan TTD dari database **TIDAK SAYA SENTUH SAMA SEKALI**.
✅ **SAYA HANYA MELAKUKAN OPRASI PASANG SUSUK PADA PDF:**
1. Saya menaikkan posisi baris nama **BACHTIAR LATIEF** dan nama Karyawan ke atas (mengurangi jarak kosong di tengah).
2. Saya menaikkan posisi gambar TTD Kacab agar berada lebih tinggi di atas nama, memberikan ruang napas yang lega dan terlihat sangat rapi (tidak *mepet*).

Silakan blok semua kode di file `CutiPage.jsx` Anda (`Ctrl + A` -> `Delete`), lalu **Copy-Paste FULL** mahakarya yang sudah dipresisikan posisinya ini:
...
export default function CutiPage() {
  ...
  const generateCutiPDF = (data) => {
    ...
    // SAKTI: Jarak kosong TTD dikurangi dari 35 menjadi 30 agar nama di bawah naik
    currentY += 30; 
    doc.setFont("helvetica", "bold"); 
    ...
    if (ttdBase64) {
      try {
        let imgFormat = 'PNG';
        if (ttdBase64.toLowerCase().includes('jpeg') || ttdBase64.toLowerCase().includes('jpg')) imgFormat = 'JPEG';
        // SAKTI: TTD dinaikkan posisinya (currentY - 31) agar tidak mepet nama
        doc.addImage(ttdBase64, imgFormat, centerKiri - 15, currentY - 31, 30, 25);
      } catch (e) {}
    }
  ...
  const exportToExcel = () => {
    let csvContent = "Peringkat,Nama Pegawai,Posisi / Jabatan,Total Cuti Terparai (Hari),Sisa Cuti (Dari 12 Hari),Total Izin (Hari)\n";
