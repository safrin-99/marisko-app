...
import { Edit, Trash2, RefreshCw, X, CheckCircle2, AlertCircle, Printer, ChevronDown, ClipboardSignature, FileText, Clock, Search } from 'lucide-react';
...
export default function KwitansiIndentPage() {
  ...
  const generatePDF = (data) => {
    ...
    const drawReceipt = (startY) => {
        ...
        doc.text("Jenis Indent", rightL, curY); doc.text(":", rightC, curY); doc.text(data.jenisIndent || '', rightV, curY);
        curY += 6;
        ...
        doc.text("No.", col1 + 5, curY + 7.5, {align: "center"}); 
        doc.text("Nama Barang", col2 + 3, curY + 7.5); 
        doc.text("Total Harga", col3 + 27, curY + 7.5, {align: "center"});
        curY += headerH;
        const startTableBody = curY;
        for (let i = 1; i <= 5; i++) {
            doc.setFontSize(9); doc.setFont("helvetica", "normal"); 
            doc.line(col1, curY, col4, curY);
            doc.text(i.toString(), col1 + 5, curY + 5, {align: "center"}); 
            if (i === 1) {
                doc.setFont("helvetica", "bold");
                doc.text(`${data.jenisIndent} - ${data.tipeMotor}`, col2 + 3, curY + 5);
                doc.setFont("helvetica", "normal");
                doc.text("Rp", col3 + 3, curY + 5); doc.text(formatRupiah(data.nominal), col4 - 3, curY + 5, { align: "right" });
            } else { doc.text("-", col2 + 3, curY + 5); doc.text("-", col3 + 27, curY + 5, { align: "center" }); }
            curY += rowH;
        }
          ...
          <div className="p-6 md:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex justify-center gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-slate-100 flex-col sm:flex-row">
                ...
                <button type="submit" disabled={isSubmitting} className={`px-8 sm:px-12 py-3 sm:py-3.5 text-white rounded-xl text-sm font-bold transition-all duration-200 active:scale-95 shadow-lg tracking-wide flex items-center justify-center w-full sm:w-auto ${isSubmitting ? 'bg-slate-400 cursor-not-allowed' : 'bg-amber-500 hover:bg-amber-600'}`}>
              ...
              <tbody className="divide-y divide-slate-100">
                {historyData
                  .filter((row) => {
                    ...
                    const searchString = `${row?.noInvoice || ''} ${row?.diterimaDari || ''} ${row?.tipeMotor || ''} ${row?.kasir || ''} ${row?.jenisIndent || ''}`.toLowerCase();
