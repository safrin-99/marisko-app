...
import { Receipt, Edit, Trash2, RefreshCw, X, CheckCircle2, AlertCircle, Printer, Search, ChevronDown, FileText, Clock, Download } from 'lucide-react';
...
export default function KwitansiPage() {
  ...
  const exportToExcel = () => {
    ...
    let csvContent = `No. Invoice,Tanggal,Diterima Dari,Leasing,Tipe Motor,Kasir,OTR,BBN,Off The Road,DP Gross,Diskon,Indent,Accesoris,Transfer,Potongan Lecet,Sisa Uang Muka\n`;
    ...
    link.setAttribute("download", `Laporan_Kwitansi_Kredit_${new Date().toISOString().slice(0,10)}.csv`);
          ...
          <div className="bg-slate-50/80 border-b border-slate-200 p-4 md:p-5 flex flex-col justify-between items-start gap-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full gap-3.5">
              <h3 className="text-lg font-bold text-slate-800 flex items-center shrink-0">
                <Receipt className="w-5 h-5 mr-2 text-indigo-600" /> Riwayat Kwitansi Kredit
              </h3>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full bg-white p-3 rounded-2xl border border-indigo-100 shadow-sm">
              <div className="relative w-full sm:w-1/3">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm font-medium focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all shadow-sm"
                />
              </div>
                ...
                {filteredHistoryData.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="text-center py-8 text-slate-400 font-medium">
                      Tidak ada data Kwitansi Kredit yang ditemukan.
                    </td>
                  </tr>
                ) : (
