  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto pb-12 space-y-8 relative">
          ...
          <div className="p-6 md:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex justify-center gap-4 pt-6 border-t border-slate-100">
                ...
                <button type="submit" className="px-12 py-3.5 bg-slate-950 text-white rounded-xl font-bold active:scale-95 shadow-lg flex items-center">
                  <ScrollText className="w-5 h-5 mr-2" />
                  {isEditing ? `Update Nota` : `Simpan & Cetak Nota ${kategoriNota}`}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="bg-slate-50/80 border-b border-slate-200 p-5 px-6 flex justify-between items-center"><h3 className="text-lg font-bold text-slate-800">Riwayat Nota {kategoriNota}</h3><button onClick={fetchHistory} className="flex items-center text-xs font-bold text-slate-700 bg-white border border-slate-300 px-4 py-2 rounded-xl active:scale-95 shadow-sm hover:bg-slate-100"><RefreshCw className={`w-3.5 h-3.5 mr-2 ${isLoading ? 'animate-spin' : ''}`} /> Refresh</button></div>
              ...
              <tbody className="divide-y divide-slate-100">
                {historyData.map((row) => {
                    ...
                    <tr key={row.id} className="hover:bg-indigo-50/40 transition-colors">
                      ...
                      <td className="px-6 py-5 text-sm font-black text-indigo-700">Rp {formatRupiah(row.total)}</td>
                      <td className="px-6 py-5">
